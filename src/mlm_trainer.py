...
def prepare_mlm_dataset(
    ...
    if problem_files:
        logging.warning(f"Failed to read {len(problem_files)} files: {problem_files}")
    ...
    if len(chunks) == 0:
        raise ValueError("No text chunks were extracted. Check your data directory.")
...
def setup_model_for_mlm_training(
    ...
    """
    Load and configure an encoder model for MLM training with optional LoRA/QLoRA.
    ...
    if use_qlora and (load_in_4bit or load_in_8bit):
        logging.info(f"Configuring quantization: 4-bit={load_in_4bit}, 8-bit={load_in_8bit}")
...
def load_trained_model(
    ...
    if is_peft_model:
        if base_model_name is None:
            raise ValueError("base_model_name required when loading PEFT model")
