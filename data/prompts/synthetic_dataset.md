...
# Task Overview
The pupose of this task is implement a synthetic dataset for SFT from a directory of pdf books.

# Task Details
Please perform the following consecutive tasks:
1. Write a function that loads and extracts the table of contents from the book.
This should simply be a hierachical version of the table of contents with keys that include the chapter or chapterr section title, the number of pages, the number of sections and the number of paragraphs in each section as well as page numbers as returned by the pdf parser.

2. Write a functions partritions a book into a set of distinc paragraphs. For each paragraph generate 1 to 3 questions about that paragraph and provide the answer from the content of the paragraph.two structured astructured JSON objects. 
